...
# trial_range = np.linspace(0.05,0.95,20)
trial_range = np.linspace(0.05,0.98,50)
DEBUG = True
...
# dataset = "airbnb"
total_nrows = 100000
print (f"Running {total_nrows}")
dataset = f"airbnb{total_nrows}"
datapath = f"data/airbnb_{total_nrows}.csv"
# datapath = "../lux-datasets/data/airbnb.csv"
...
#     with open(f"tmp_{dataset}/airbnb_gt_{action}.pkl",'wb') as f:
#         pkl.dump(map1,f)
    ...
    if ("airbnb" in dataset):
        ...
        for seed in seed_lst:
            ...
            for action in q1_recs2.keys():
                map1 = pkl.load(open(f"tmp_{dataset}/airbnb_gt_{action}.pkl",'rb'))
                map2 = convert_vlist_to_hashmap(q1_recs2[action])
                with open(f"tmp_{dataset}/airbnb_nfrac{nfrac:.2f}_{action}_{seed}.pkl",'wb') as f:
                    pkl.dump(map2,f)
                if len(map2)==15 :
