...
# dataset = "airbnb"
total_nrows = 50000
dataset = f"synthetic{total_nrows}"
datapath = f"data/synthetic_{total_nrows}.csv"
# datapath = "../lux-datasets/data/synthetic.csv"
...
#     with open(f"tmp_{dataset}/synthetic_gt_{action}.pkl",'wb') as f:
#         pkl.dump(map1,f)
    ...
    if ("synthetic" in dataset):
        ...
        for seed in seed_lst:
            ...
            for action in q1_recs2.keys():
                map1 = pkl.load(open(f"tmp_{dataset}/synthetic_gt_{action}.pkl",'rb'))
                map2 = convert_vlist_to_hashmap(q1_recs2[action])
                with open(f"tmp_{dataset}/synthetic_nfrac{nfrac:.2f}_{action}_{seed}.pkl",'wb') as f:
                    pkl.dump(map2,f)
                if len(map2)==15 :
